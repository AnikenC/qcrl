...
class ResonatorEnv(gym.Env):
    ...
    def step(self, action):
        ...
        batched_drive_res = self.max_drive_amplitude * jnp.array(
            ...
            + 1.0j * action[:, self.n_actions : 2 * self.n_actions],
            dtype=self.complex_dtype,
        )
        batched_res_state = jnp.zeros((self.batch_size, 1), dtype=self.complex_dtype)
        # batched_res_state = self.max_init_state_amplitude * jnp.array(
        #    action[:, -2] + 1.0j * action[:, -1],
        #    dtype=self.complex_dtype,
    ...
    def single_eval(self, res_drive, res_state):
        ...
        freqs = jnp.linspace(-5.0, 5.0, self.n_actions, dtype=self.float_dtype)
        fft_vals = jnp.fft.fft(res_drive)
        fft_shifted = jnp.abs(jnp.fft.fftshift(fft_vals))
        indices = jnp.where(
            fft_shifted > self.threshold * jnp.max(fft_shifted), size=self.n_actions
        )[0]
        min_index = indices[0]
