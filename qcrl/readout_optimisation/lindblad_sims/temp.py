...
from qutip.sesolve import (
    _sesolve_list_func_td,
    _sesolve_list_str_td,
    _sesolve_list_td,
    _sesolve_func_td,
    _sesolve_const,
)
...
def _mesolve_list_str_td(H_list, rho0, tlist, c_list, e_ops, args, opt, progress_bar):
    ...
    if not opt.rhs_reuse or config.tdfunc is None:
        ...
        code = compile(
            "from " + config.tdname + " import cy_td_ode_rhs", "<string>", "exec"
        )
        exec(code, globals())
...
def _mesolve_const(H, rho0, tlist, c_op_list, e_ops, args, opt, progress_bar):
    ...
    if isket(rho0):
        ...
        if len(c_op_list) == 0 and isoper(H):
            return _sesolve_const(H, rho0, tlist, e_ops, args, opt, progress_bar)
    ...
    if type(args) is dict:
        ...
        for key in args:
            if isinstance(args[key], Qobj):
                if isoper(args[key]):
                    new_args[key] = -1j * (spre(args[key]) - spost(args[key]))
                else:
    ...
    for t_idx, t in enumerate(tlist):
        ...
        if opt.store_states or expt_callback:
            rho.data = dense2D_to_fastcsr_fmode(
                vec2mat(r.y), rho.shape[0], rho.shape[1]
            )
...
def odesolve(H, rho0, tlist, c_op_list, e_ops, args=None, options=None):
    ...
    warnings.warn(
        "odesolve is deprecated since 2.0.0. Use mesolve instead.", DeprecationWarning
    )
    ...
    if (c_op_list and len(c_op_list) > 0) or not isket(rho0):
        if isinstance(H, list):
            output = _mesolve_list_td(
                H, rho0, tlist, c_op_list, e_ops, args, options, BaseProgressBar()
            )
        if isinstance(H, (types.FunctionType, types.BuiltinFunctionType, partial)):
            output = _mesolve_func_td(
                H, rho0, tlist, c_op_list, e_ops, args, options, BaseProgressBar()
            )
        else:
            output = _mesolve_const(
                H, rho0, tlist, c_op_list, e_ops, args, options, BaseProgressBar()
            )
    else:
        if isinstance(H, list):
            output = _sesolve_list_td(
                H, rho0, tlist, e_ops, args, options, BaseProgressBar()
            )
        if isinstance(H, (types.FunctionType, types.BuiltinFunctionType, partial)):
            output = _sesolve_func_td(
                H, rho0, tlist, e_ops, args, options, BaseProgressBar()
            )
        else:
            output = _sesolve_const(
                H, rho0, tlist, e_ops, args, options, BaseProgressBar()
            )
