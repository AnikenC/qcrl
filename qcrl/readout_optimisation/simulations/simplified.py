...
def single_eval(
    ...
    stepsize_controller = PIDController(
        rtol=1e-4, atol=1e-7, pcoeff=0.4, icoeff=0.3, dcoeff=0.0, jump_ts=ts_actions
    )
    saveat = SaveAt(
        steps=True,
        # fn=lambda t, y, args: jnp.array([y[0], y[1], y[2], y[3]], dtype=float_dtype),
    )
    ...
    return sol_g.ys, sol_e.ys, sol_g.stats, sol_e.stats, sol_g.ts, sol_e.ts
...
single_g, single_e, g_stats, e_stats, _, __ = jitted_single(
    in_state, in_drive_res, in_drive_trans, in_res_freq, in_trans_freq
)

start = time.time()
second_g, second_e, g_stats, e_stats, _, __ = block_until_ready(
    jitted_single(in_state, in_drive_res, in_drive_trans, in_res_freq, in_trans_freq)
)
now = time.time()

batched_eval = jit(vmap(single_eval, in_axes=1))
batched_, batched__, a_, b_, _, __ = batched_eval(
    b_state, b_drive_res, b_drive_trans, b_res_freq, b_trans_freq
)
print(f"time taken for single g + e: {(now - start)*1e3}ms")
print(f"for g")
...
(
    ...
    batched_eval(b_state, b_drive_res, b_drive_trans, b_res_freq, b_trans_freq)
)

now = time.time()
print(f"time taken for single g + e after batching: {(now - start)/n_trials*1e3}ms")
print(f"for g")
...
infinities_g_0 = jnp.isposinf(batched_g_time[0])
infinities_e_0 = jnp.isposinf(batched_e_time[0])

where_inf_g = jnp.array(jnp.where(infinities_g_0 == True))
where_inf_e = jnp.array(jnp.where(infinities_e_0 == True))

deleted_g = jnp.delete(batched_g_time[0], where_inf_g)
deleted_e = jnp.delete(batched_e_time[0], where_inf_e)

batch_g_element = jnp.delete(batched_g[0], where_inf_g, axis=0)
batch_e_element = jnp.delete(batched_e[0], where_inf_e, axis=0)

print(f"time steps without inf g: {deleted_g}, ")
print(f"time steps without inf e: {deleted_e}, shape: {deleted_e.shape}")

for i, t in enumerate(deleted_g[0:100]):
    print(f"time g: {t}")
    print(f"vector field value: {batch_g_element[i]}")

for i, t in enumerate(deleted_e[0:100]):
    print(f"time e: {t}")
    print(f"vector field value: {batch_e_element[i]}")
