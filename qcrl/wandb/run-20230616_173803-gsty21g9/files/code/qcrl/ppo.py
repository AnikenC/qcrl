...
if __name__ == "__main__":
    ...
    for update in tqdm(range(1, args.num_updates + 1)):
        ...
        if args.track:
            writer.add_scalar(
                "charts/mean_reward", np.asarray(info["mean reward"]), update
            )
            writer.add_scalar(
                "charts/mean_reward", np.asarray(info["mean max photon"]), update
            )
            writer.add_scalar(
                "charts/mean_reward", np.asarray(info["mean max separation"]), update
            )
            # writer.add_scalar(
